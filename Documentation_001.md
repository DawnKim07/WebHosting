...
개발용 컴퓨터 내부에 웹 브라우저와 웹 서버가 같이 존재하지만, 브라우저와 서버는 기본적으로 80번과 443번 포트를 통한 HTTP 및 HTTPS 통신만 가능하기 때문에 내부적인 통신은 불가능하다. 따라서 개발용 컴퓨터의 브라우저에 도메인 주소를 입력하게 되면, DNS 해석을 위해 DNS 서버에 연결된다. DNS 서버에서 해석한 결과 개발용 컴퓨터의 외부IP 주소가 산출되면, DNS 서버는 다시 개발용 컴퓨터로 통신을 보낸다. 그러나 여기에서 문제가 발생한다. 웹 서버는 HTTP(S) 통신 요청을 보낸 기기가 웹 서버 자신이 탑재된 기기와 동일하다는 것을 알고 외부IP주소를 내부IP주소(127.0.0.1)로 바꾸고 통신을 시도하지만, 이 때 역시 포트를 통해 외부로 통신을 거칠 수밖에 없다. 그러나 이미 IP주소는 내부IP주소로 바뀐 상태고, `127.0.0.1`이라는 외부IP주소를 가진 기기는 웹 상에 존재하지 않기 때문에 개발용 컴퓨터의 브라우저는 요청한 웹페이지의 응답을 받는 데 실패한다. 이 때 문제를 해결해주는 것이 바로 vhosts 문서이다. 이 문서는 웹 서버가 접속 대상으로 설정한 내부IP주소를 다시 외부IP주소로 바꿔주는 일종의 DNS 설정 역할을 한다. 웹서버가 외부IP주소를 대상으로 정하면, 외부 DNS 서버를 통해 정상적으로 브라우저와 통신이 가능하게 되는 것이다.
